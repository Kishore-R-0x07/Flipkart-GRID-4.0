...
def precision_hover(tracker, function = 'hover'):
    """
    Descends to hover_height, over the required aruco
    Checks for object, picks it up if it's there
    Rises back to 5m
    function: hover/drop

    Returns
    loc: current lat and lon, and 5m above ground
    """
    loc = None
    while True:                
        ...
        if marker_found:
            ...
            if time.time() >= time_0 + 1.0/freq_send:
                ...
                if uav_location.alt <= hover_height:
                    loc = LocationGlobalRelative(vehicle.location.lat, vehicle.location.lon, rise_height)
                    break
    ...
    while True:
        global markerobj_found                
        markerobj_found, xobj_cm, yobj_cm, zobj_cm = tracker_object.track(loop=False)
        # Marker object found 
        # CODE HERE
        if markerobj_found:
            object_hover() # Switches over to object's aruco, descends, and activates EM 
            vehicle.simple_goto(LocationGlobalRelative(vehicle.location.lat, vehicle.location.lon, rise_height))
            while True:
                print(" Altitude: ", vehicle.location.global_frame.alt)
                # Break and return from function just below target altitude.
...
def object_hover(function):
    """
    Descends close to the object's aruco (~15 cm) and picks up the object
    HAVE TO CORRECT FOR ARUCO GOING OUT OF CAMERA BOUNDS
    CAN PUT A WAIT_TIME FOR ELECTROMAGNET
    """

    while True:                
        
        marker_found, x_cm, y_cm, z_cm = tracker_object.track(loop=False)
        ...
        if marker_found:
            ...
            if time.time() >= time_0 + 1.0/freq_send:
                ...
                if uav_location.alt <= pickup_height:

                    #ACTIVATE EM
                    GPIO.output(pin, GPIO.HIGH)
                    time.sleep(5)
                    break # right after pickup
...
loc_drop = precision_hover(tracker=tracker_drop, function='drop') # NEEDS TO BE PROPERLY HANDLED - GPS LOCK @ 5m/2m
# loc_drop = LocationGlobalRelative(vehicle.location.lat, vehicle.location.lon, vehicle.location.alt)
# put the above line inside the function - DONE

#------------------------------------------------------------------------

loc1 = precision_hover(tracker=tracker1) 
...
if markerobj_found:
    ...
    _ = precision_hover(tracker=tracker_drop, function='drop')
    # DROPPED, Right now at 5m
